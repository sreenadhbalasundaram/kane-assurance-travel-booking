...
### FR-6: Payment Failure Fixture`r`n`r
The demo may expose an opt-in deterministic payment failure fixture for negative-path coverage. If enabled, the page must retain the user on StayPay, show an actionable error, and permit retry with another payment method.`r`n`r
## Open Questions
